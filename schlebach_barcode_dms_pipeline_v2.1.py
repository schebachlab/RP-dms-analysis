...
def qualityfilter3_original(data, seqstart, seqlen, threshold=30, errors=1):
    filtered_data = []
    avg_scores = []
    passed = 0
    failed = 0
...
def qualityfilter3_dict(data, seqstart, seqlen, threshold, errors=1):
    filtered_data = {}
    avg_scores = []
    passing_coords = []
    passed = 0
...
def qualityfilter3b(data_r1, data_r2, read_seqstart, read_length_r1, read_length_r2, threshold, errors=1):
    
    filtered_data_r1, avg_scores_r1, passed_r1, num_reads_r1, passing_coords_r1 = qualityfilter3_dict(data_r1, read_seqstart, read_length_r1, threshold, errors)
    filtered_data_r2, avg_scores_r2, passed_r2, num_reads_r2, passing_coords_r2 = qualityfilter3_dict(data_r2, read_seqstart, read_length_r2, threshold, errors)
...
def parse_config_file(config_file):
    ...
    read_length_r1 = parser.getint('Library and NGS parameters', 'R1 Read length')
    if reverse_read_choice == True:
        read_length_r2 = parser.getint('Library and NGS parameters', 'R2 Read length')
    else:
        read_length_r2 = 'NA'
...
if __name__ == '__main__':
    ...
    print('R1 Read length: ', read_length_r1, file=logfile)
    if reverse_read_choice == True:
        print('R2 Read length: ', read_length_r2, file=logfile)
    ...
    if reverse_read_choice == True:
        for facs_bin in range(num_bins):
            ...
            filtered_data_dict, avg_scores_r1, avg_scores_r2, passed_r1, passed_r2, raw_reads_r1, raw_reads_r2 = qualityfilter3b(r1_raw, r2_raw, 0, read_length_r1, read_length_r2, qscore_threshold, errors)
        ...
        for facs_bin in range(num_bins):
            ...
            r1_raw = fastq_parser(path_fastq_r1[facs_bin])
            filtered_data, avg_scores_r1, passed_r1, raw_reads_r1 = qualityfilter3_original(r1_raw, 0, read_length_r1, qscore_threshold, errors)
            print('Quality score threshold: ', qscore_threshold, file=logfile)
            print('Number of R1 reads before filtering, bin', num_alpha_dict[facs_bin], ': ', raw_reads_r1, file=logfile)
            print('Percent passing reads, R1 reads, bin', num_alpha_dict[facs_bin], ': ', passed_r1, file=logfile)
            print('Average quality score, passing R1 reads, bin', num_alpha_dict[facs_bin], ': ', avg_scores_r1, file=logfile)
    ...
    if (reverse_read_choice, lib_diag_choice) == (True, True):
        ...
        filtered_data_dict, avg_scores_r1, avg_scores_r2, passed_r1, passed_r2, raw_reads_r1, raw_reads_r2 = qualityfilter3b(r1_lib_raw, r2_lib_raw, 0, read_length_r1, read_length_r2, qscore_threshold, errors)
        print('Quality score threshold: ', qscore_threshold, file=logfile)
    ...
    if (reverse_read_choice, lib_diag_choice) == (False, True):
        r1_lib_raw = fastq_parser(path_lib_r1)
        filtered_data_lib, avg_scores_r1, passed_r1, raw_reads_r1 = qualityfilter3_original(r1_lib_raw, 0, read_length_r1, qscore_threshold, errors)
        print('Quality score threshold: ', qscore_threshold, file=logfile)
        print('Number of R1 reads before filtering, lib file: ', raw_reads_r1, file=logfile)
